...
# def __init__():
#     coords = read_points()  # get coords of the points 
...
#         if perms[i] != 0:
#             make_photo(i) # take a new photo
#             print("Zrobilem zdjecie nr: " + str(i))
            
#         ser.write(commands[int((i)/4)].encode())

#         if perms[i] != 0:
#             take_colors(i)  # read colors from taken photo   
#             assign_colors(colors_on_photo, perms[i])   # make an transformations of the perms
#             draw_cube(colors_on_cube, i)  # draw flat view of the photos
#             draw_points_on_photo(i)
...
def read_points():
    ...
    coords = coords_down + coords_up
    return coords

def make_photo(number):
    # history.save_history()
    make_a_photo.make_a_photo("/dev/video2", "/dev/video0", number)

def draw_points_on_photo(image_number):
    cube_transformation = _transformations_table('NORMAL') 
    photo_transformation = _transformations_table('PHOTO')
    draw_points_on_image.draw_points_on_image(coords_up, PATH + 'img/cube_photos/cube_up_'+str(image_number)+'.png', cube_transformation, photo_transformation, 'up')
    draw_points_on_image.draw_points_on_image(coords_down, PATH + 'img/cube_photos/cube_down_'+str(image_number)+'.png', cube_transformation, photo_transformation, 'down')

def take_colors(image_number):
    """
    This function take an colors from photo in given points.
    """
    colors_down = take_colors.take_colors(coords_down, PATH + 'img/cube_photos/cube_down_'+str(image_number)+'.png')
    colors_up = take_colors.take_colors(coords_up, PATH + 'img/cube_photos/cube_up_'+str(image_number)+'.png')
    colors_on_photo = (colors_down + colors_up)

def assign_colors(colors_on_photo, rotation):
    """
    This function make an projection of read colors on given transformation of the cube.
    """
    cube_transformation = _transformations_table(rotation)
    photo_transformation = _transformations_table('PHOTO')
    ...
    colors_on_cube, cube_transform =  projected_colors, cube_transformation
...
def draw_cube(colors, number=''):
    ...
    for i in range(len(cube_draw_points_y)):
        ...
        for j in range(3):
            ...
            draw.text(((x*40)+16,(y*40)+16),fill=(100,100,100), text=str(cube_transform.index(n+1)+1))
            n+=1
    im.save(PATH+'img/flat_views/flat_view'+ str(number) +'.png')
    im.close()
...
def random_scramble(cube):
    ...
    for rotation in scramble:
        if rotation == 'F':
            transformation_table = _transformations_table('R1')
        elif rotation == 'B':
            transformation_table = _transformations_table('O1')
        elif rotation == 'R':
            transformation_table = _transformations_table('Y1')
        elif rotation == 'L':
            transformation_table = _transformations_table('W1')
        elif rotation == 'U':
            transformation_table = _transformations_table('B1')
        elif rotation == 'D':
            transformation_table = _transformations_table('G1')

        bufor = colors_after_scramble[:]
        for j in range(len(transformation_table)):
            colors_after_scramble[j] = bufor[transformation_table[j]-1]

    return colors_after_scramble

def _transformations_table(t):
